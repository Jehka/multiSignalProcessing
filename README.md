...
graph LR
    ...
    subgraph Physical Sensors
        direction TB
        S1[MQ-3<br>Alcohol/VOC]
        S2[MQ-135<br>Air Quality]
        S3[LDR<br>Light Level]
    end

    %% Step 2: SPI Frontend
    subgraph SPI Front-End
        direction TB
        ADC[External SPI ADC<br>AD7606B]
        SPI_M[spi_adc_master_v2<br>16-bit padded]
        MUX[channel_mux_fsm<br>Round-Robin Arbiter]
    end

    S1 & S2 & S3 -->|Analog V| ADC
    ADC <-->|SPI Bus| SPI_M
    SPI_M --> MUX

    %% Step 3: AXI Lanes
    subgraph AXI-Lite Processing Lanes
        direction TB
        subgraph CH0 Lane
            AM0[axil_master_ch 0]
            FIR0[axi_moving_avg]
            TC0[thresh_comparator]
            AM0 <--> FIR0
            AM0 <--> TC0
        end
        subgraph CH1 Lane
            AM1[axil_master_ch 1]
            FIR1[axi_moving_avg]
            TC1[thresh_comparator]
            AM1 <--> FIR1
            AM1 <--> TC1
        end
        subgraph CH2 Lane
            AM2[axil_master_ch 2]
            FIR2[axi_moving_avg]
            TC2[thresh_comparator]
            AM2 <--> FIR2
            AM2 <--> TC2
        end
    end
    ...
    %% Step 4: DSP Core
    subgraph DSP Fusion Core
        DSP[dsp_core.v<br>- Leaky Integrators<br>- Priority Encoder<br>- Atomic Trigger]
    end

    AM0 -->|fir_result + irq| DSP
    AM1 -->|fir_result + irq| DSP
    AM2 -->|fir_result + irq| DSP

    %% The crucial feedback loop
    DSP -.->|Dynamic thresh_hi / lo| AM0
    DSP -.->|Dynamic thresh_hi / lo| AM1
    DSP -.->|Dynamic thresh_hi / lo| AM2

    %% Step 5: Outputs
    subgraph System Outputs
        direction TB
        ALARM[Alarm Level<br>00: CLEAR<br>01: WARNING<br>10: DANGER<br>11: CRITICAL]
        ACH[Alarm Channel]
        EV[Event Valid Pulse]
    end
